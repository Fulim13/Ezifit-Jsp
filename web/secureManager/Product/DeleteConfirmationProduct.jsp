...
<html>
    ...
    <body>
        ...
        <div class="col-2">
            ...
            <h1 style="color: #8B0000">DELETE PRODUCT</h1>
...
%>
        <table class="productreview">
            <tr>
                <td class="cart">                 
                    <img id="img" src="data:image/jpg;base64,<%=product.getBase64Image()%>">
                    <div class="descriptionC">
                        <div class="name"><%= product.getProdName()%></div>
                        <div>Size: <%= product.getSize()%></div>
                        <div>Quantity: <%= product.getQuantity()%></div>
                    </div>
                </td>
                <td class="customerReview">
                    <form action="../../DeleteProduct" method="post" onsubmit="return window.confirm('Are you sure you want to delete this item?')">
                        
                        <input type="hidden" name="prod_picture" value="">
                        <div class="input-box">
                            <label class="details">Product ID</label>
                            <input type="text" class="form-control" name="prod_id"
                                   value="<%=product.getProdId()%>" readonly="readonly"/>    
                        </div>

                        <div class="input-box">
                            <label class="details">Product Name</label>
                            <input type="text" class="form-control" name="prod_name" value="<%=product.getProdName()%>" readonly="readonly"/>    
                        </div>

                        <div class="input-box">
                            <label class="details">Description</label>
                            <input type="text" class="form-control" name="description" value="<%=product.getDescription()%>" readonly="readonly"/>    
                        </div>

                        <div class="input-box">
                            <label class="details">Price</label>                           
                            <input type="text" class="form-control" name="price"
                                   value="<%=String.format("%.2f", product.getPrice())%>"  readonly="readonly"/>    
                        </div>

                        <div class="input-box">
                            <label class="details">Quantity</label>
                            <input type="text" class="form-control" name="quantity"
                                   value="<%=String.format("%d", product.getQuantity())%>" readonly="readonly"/>    
                        </div>

                        <div class="input-box">
                            <label class="details">Weight</label>
                            <input type="text" class="form-control" name="weight"
                                   value="<%=String.format("%.2f", product.getWeight())%>" readonly="readonly" />    
                        </div>

                        <div class="input-box">
                            <label class="details">Status</label>
                            <input type="text" class="form-control" name="status"
                       value="<%= product.getStatus()%>"readonly="readonly" />    
                        </div>

                        <div class="input-box">
                            <label class="details">Cost</label>
                            <input type="text" class="form-control" name="cost"
                       value="<%= product.getCost()%>"readonly="readonly" />    
                        </div>
                        
                        <div class="input-box">
                            <label class="details">Size</label>
                            <input type="text" class="form-control" name="size"
                       value="<%= product.getSize()%>"readonly="readonly" />    
                        </div>
                        
                        <div class="input-box">
                            <label class="details">Category</label>
                            <input type="text" class="form-control" name="size"
                       value="<%= product.getCategory()%>"readonly="readonly" />    
                        </div>
                        
                        <div class="feedback" style="color:#757575;">Description</div>
                        <textarea placeholder="<%=product.getDescription()%>"  name="description" rows="6" cols="60" class="form-control" readonly="readonly"><%=product.getDescription()%></textarea>    
